...
const filePath = './src/components/kol/KOLOverview.tsx';
let content = fs.readFileSync(filePath, 'utf8');

// Find the line with imports and add Calendar import
content = content.replace(
  "Clock,\n  ArrowUp,\n  ArrowDown\n} from 'lucide-react';",
  "Clock,\n  ArrowUp,\n  ArrowDown,\n  Calendar,\n  UserCheck,\n  Sparkles\n} from 'lucide-react';"
);

// Find the optimizationInsights array closing and add new data
const insertionPoint = content.indexOf('  ];');
const beforeInsertion = content.substring(0, insertionPoint + 4);
const afterInsertion = content.substring(insertionPoint + 4);
...
const updatedContent = beforeReturn + newDataStructures + '\n\n' + afterReturn;

// Now find where to insert the new visualization sections
// We'll add them after the Optimization Insights section
const optimizationInsightsEnd = updatedContent.indexOf('          </div>\n        </div>\n      </div>\n    </div>\n  );');
const beforeOptEnd = updatedContent.substring(0, optimizationInsightsEnd);
const afterOptEnd = updatedContent.substring(optimizationInsightsEnd);

const newVisualizationSections = `          </div>
                  ...
                  <div key={index} className="relative">
                    <div className={`p-4 border rounded-lg transition-all hover:shadow-md ${
                      ...
                      <div className="relative h-8 bg-gray-200 rounded-full overflow-hidden">
                        ...
                        >
                          <div className="absolute inset-0 bg-white opacity-20 transform -skew-x-12 -translate-x-full animate-shine"></div>
                        </div>
...
// Replace the content
const finalContent = beforeOptEnd + newVisualizationSections + afterOptEnd;

// Add the CSS animation for the shine effect at the end of the file
const animationCSS = `
...
const closingIndex = finalContent.lastIndexOf('export default KOLOverview;');
const beforeClosing = finalContent.substring(0, closingIndex);
const afterClosing = finalContent.substring(closingIndex);

const completeContent = beforeClosing + afterClosing;

// Write the file
fs.writeFileSync(filePath, completeContent, 'utf8');
console.log('Successfully updated KOLOverview.tsx with new visualizations!');